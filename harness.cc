...
static GeoIP* initGeoHandle(const char* file_path)
{
	return GeoIP_open(file_path, GEOIP_MEMORY_CACHE);
}
...
static void get_geo_data(const string& ip, geo *g)
{
	if (g_geo_handle == NULL || ip.empty()) {
		LOG(ERROR) << "g_geo_handle is NULL or ip is NULL";
		return;
	}
	g->ctry = "";
	g->region = "";
	...
	gir = GeoIP_record_by_name(g_geo_handle, ip.c_str());
	if (NULL != gir)
	{
		/*check NULL for each of the fields if necessary --- chk maxmind code*/
		if (gir->country_code != NULL){
			g->ctry = gir->country_code;
		} 
		if (gir->region != NULL){
			g->region = gir->region;
		}
...
static vector<string> tokenize(char *str)
{
	if (str == NULL || str[0] == '\0') {
...
static impr_list* getImprList(const char* path)
{
	...
	while (token != NULL) {
		...
		/*
			 ...
			 */
		const auto& v = tokenize(token);
		if (v.size() != MAX_TSV) {
			LOG(ERROR) << "tokenize returned " << v.size() << " whereas # of tsvs = " << MAX_TSV;
			free(list);
			free(buf);
			return NULL;
		}

		const string& ip = v[IP_IDX];
		geo g;
		get_geo_data(ip, &g);
		...
		imp.input[P2P_IDX] = strdup(v[13].c_str());
		imp.input[MINP2P_IDX] = strdup(v[14].c_str());
		imp.input[CP_IDX] 	= strdup(v[15].c_str());
		imp.input[PF_IDX] 	= strdup(v[16].c_str());
...
static void* update_(void *arg)
{
	...
	while (true) {
		...
		UpdatePF(&(ctx->adflex_db_handle));
		fprintf(stderr, "\nPlatform_fee updated from db\n");
		// Update the OS List
		UpdateOSList(&(ctx->adflex_db_handle));
		fprintf(stderr, "\nOSList updated from db\n");
		// Update the Browser List
		UpdateBrowserList(&(ctx->adflex_db_handle));
		fprintf(stderr, "\nBrowserList updated from db\n");
		// Update MBF
		UpdateMBF(&(ctx->adflex_db_handle));
		fprintf(stderr, "\nMBF model updated from db\n");
...
int main(int argc, char *argv[])
{
	...
	if (argc != 7) {
		fprintf(stderr, "\nERROR usage <exe> <# of threads> <# of iterations> <impr_file_path> <output_file_path> <tree_file_path> <GeoFilePath>..exiting\n");
		exit(1);
	}
	...
	const char *tree_file_path = argv[5];
	const char *geo_file_path = argv[6];


	g_geo_handle = initGeoHandle(geo_file_path);
	if (g_geo_handle == NULL) {
		LOG(ERROR) << "initGeoHandle failed for " << geo_file_path;
		exit(1);
	}
